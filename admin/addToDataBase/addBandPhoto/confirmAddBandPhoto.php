...
// set addTourDates sessions
    ...
    $_SESSION['addBandPhotos']['date']= $_POST['date'];
    $_SESSION['addBandPhotos']['state_country']= $_POST['state_country'];
    

if(empty($_SESSION['addBandPhotos']['date']) || empty($_SESSION['addBandPhotos']['state_country'])) {

    header("Location:addBandPhotos.php?signup=empty");
 ...
 // check if input characters are valid//
        ...
        if(!preg_match("/^[a-zA-Z]/", $_SESSION['addBandPhotos']['state_country'])){

          header("Location:addBandPhotos.php?signup=char");
          exit();
        } 
...
<script src="/metallicaTourProject/js/ajaxUnlinkImage.js" defer></script>
<meta charset = "utf-8">
</head>
...
<section class="c-admin-confirm-addDeleteOrEdit-to-db h-flex h-position">
  ...
  <!-- because the parent is position relative we have a position absolute on this of top, left and right of 0 to make the icon align at the top left hand side of the page  -->
    <div class="c-back-page-icon-container">
        ...
        <a href="addBandPhotos.php" class="c-js-back-page-icon-link"></a>
...
<p class="c-admin-confirm-addDeleteOrEdit-to-db-desc">
    ...
    <!-- these links take out their decoration but colour the text our white -->
      ...
      <!-- end of colour the text our white-->
       ...
       | 
       <!-- these links take out their decoration but colour the text our white -->
       <a class="c-admin-confirm-addDeleteOrEdit-to-db-links" href="/metallicaTourProject/admin/logIn/admin.php">Back to admin panel</a>
        <!-- end of colour the text our white-->
    </p>
...
<!---this helper class below is for the card design elements like the band photos, the news stories and the music albums to keep the overflow hidden---->
    <article class="h-overflowH">
        ...
        <img class="c-latest-news-articles-photo-img h-margin-centre h-padding-left-to-right" src="/metallicaTourProject/img/<?php echo $_SESSION['addBandPhotos']['img']; ?>" />
 ...
 <!---this class aligns the latest news text through padding, we also have a helper class of aligining text to centre here because again it is just one item in the grid and to make it look better we centre it---->
        <div class="c-latest-news-articles-text-container h-text-align ">
            ...
            <div class="c-latest-news-articles-date"><?php echo  $_SESSION['addBandPhotos']['date']; ?>
        </div>
      ...
      <!-- this paragraph has no class attached as we want to keep the max width set up at the beginning, because when it is just one item we need the text to be centred and not span the whole page -->
        <p> <?php echo  $_SESSION['addBandPhotos']['state_country'] ;?> </p>
...
<p class="c-admin-confirm-addDeleteOrEdit-to-db-desc">
    ...
    <!-- end of colour the text our white-->
      ...
      <!-- end of colour the text our white-->
       ...
       <a class="c-js-admin-confirm-addDeleteOrEdit-to-db-links" href="/metallicaTourProject/admin/logIn/admin.php">Back to admin panel</a>
        <!-- end of colour the text our white-->
         <!-- END OF JAVASCRIPT FUNCTIONALITY -->
    </p>
    <!-- end of no text here, that is done by the links, it is just to colour the separator our red   -->
...
if($imageName!=""){
	...
	$_SESSION['addBandPhotos']['img'] = $imageName;

// this is the whole file path e.g race-ur-self1.jpg in the session

	$target_dir = $_SERVER['DOCUMENT_ROOT']."/metallicaTourProject/img/";
      // as it says ../img/
...
instructionsOnWhereToGo("addBandPhotos.php");
// if everything is ok, try to upload file
} 

else if (move_uploaded_file($imageTempFile, $target_file)) {
        
  successMessage("enterNewBandPhoto.php", "addBandPhotos.php");
